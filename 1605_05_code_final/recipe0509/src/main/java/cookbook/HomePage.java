...
public class HomePage extends WebPage {
	...
	private IModel<Contact> selected = new Model<Contact>();

	public HomePage(final PageParameters parameters) {
		List<IColumn<Contact>> columns = new ArrayList<IColumn<Contact>>();
		columns.add(new ClickablePropertyColumn<Contact>(Model.of("Name"),
				"name") {

			@Override
			protected void onClick(IModel<Contact> clicked) {
				selected.setObject(clicked.getObject());
			}
		...
		add(new Label("name", new PropertyModel<String>(selected, "name")));
		add(new Label("email", new PropertyModel<String>(selected, "email")));
		add(new Label("phone", new PropertyModel<String>(selected, "name")));
	}
